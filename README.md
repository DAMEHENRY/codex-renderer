...
- **Reasoning effort override**: Adjusts the reasoning depth of reasoning models. Passed as a verified Codex configuration override using `-c model_reasoning_effort="<value>"`. Valid overrides are `none`, `minimal`, `low`, `medium`, `high`, `xhigh`.
...
2. **Open Codex Chat**: Click the terminal ribbon icon (or run the command `Open Codex Chat` from the command palette) to open the sidebar chat pane.
